...
model_path = 'D:/MabaniBinaei/Project/yolov5_ABS-main/yolov5_ABS-main/yolov5s.pt'

device = "cpu"  # for cpu
model = torch.hub.load('ultralytics/yolov5', 'custom', path=model_path)  # local model
model.to(device)

# لیست کلاس‌های حیوانات در دیتاست COCO
animal_classes = [16, 17, 18, 19, 20, 21, 22, 23, 24]  # ID کلاس‌های حیوانات

# Load the video
video = cv2.VideoCapture('D:/MabaniBinaei/Project/yolov5_ABS-main/yolov5_ABS-main/trimmedVideo_11.mp4')
...
while True:
    ...
    for detection in detections:
        ...
        ymax = detection[3]
        score = detection[4]
        class_id = int(detection[5])  # کلاس تشخیص داده شده

        # اگر کلاس تشخیص داده شده جزو حیوانات باشد
        if class_id in animal_classes and score >= 0.4:
